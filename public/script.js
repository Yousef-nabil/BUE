  ...
  try {
    ...
    const payloadMap = {
      nema17: {
        ...
        motor_one_dircetion:
          direction === "anticlockwise" ? "anti-clock-wise" : "clock-wise",
      },
      nema23: {
        motor_two_angle: angle.toString(),
        motor_two_dircetion:
          direction === "clockwise" ? "clock-wise" : "anti-clock-wise",
      },
    };
...
async function initializeSteps() {
  try {
    ...
    console.log(
      `Initialized steps: NEMA17 = ${nema17Steps}, NEMA23 = ${nema23Steps}`
    );
  } catch (error) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  controls.forEach((control) => {
    ...
    const anticlockwiseBtn = control.querySelector(
      ".dir[onclick*='anticlockwise']"
    );
